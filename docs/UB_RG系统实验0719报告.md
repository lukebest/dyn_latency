...
> **执行状态：完成。** 网络任务 80 个，完成 80、失败 0；系统配置 52 个，完成 52、因网络输入失败 0。拓扑为场景1（单层 switch）。
...
| sys3 | 14 | controls, main | 1 | packet_spray, ub_rg | 256 | 0, 0.5, 1 | 128 | 60 | 1, 2, 4 |
...
**设计（§4.3.2）**：仍为 Wide-EP；全局 batch 切成 m 个 microbatch，计算/通讯双 stream 做 TBO ping-pong，期望通讯被计算掩盖。主配置 m=2；对照 m=1（退化为不掩盖）与 m=4。观测：相对串行的 speedup，以及通讯事件与计算重叠比例（mask）。

**本轮矩阵**：场景1、B=256、EP=128、L=60；主点 m=2 且 S∈{0, 0.5, 0.9}；对照 m∈{1, 4} 且 S∈{0.5, 0.9}。实收 summary **14** 个。

**本轮结论**：

- Packet Spray、S=0.5：m=2/m=4 相对 m=1 为 1.48× / 2.19×（mask：5.4% 通讯重叠 / 16.0% 通讯重叠）。加速是「切小 MB 后 CCT 变化 + 双 stream 重叠」的净效应，本矩阵未做因素消融。

- 高倾斜下掩盖变难：Spray S=0.9 时 m=2 相对 m=1 仅 0.99×（mask：1.6% 通讯重叠）。

- `ub_rg`、S=0.5：m=2/m=4 speedup 为 1.31× / 0.86×（m=4 相对串行退化）；通讯重叠均为 0% 量级。TBO 并非必然获益。
...
**本轮矩阵**：场景1、B=256、L=60、placement=`role_packed`；7:1 扫 m∈{1,2,4} 与 Zipf；另含 1:1、31:1 对照。实收 summary **14** 个。

**本轮结论**：

- 双向掩盖：14/14 个样本标记为未通过；无样本显示通讯被计算完全隐藏。
...
![sys3 sys3_tc_throughput](ub_rg_system_figures/sys3_tc_throughput.svg)
